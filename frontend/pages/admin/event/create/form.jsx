...
export default function CreateEvent() {
    ...
    const getRoomsAvailable = (startDate, endDate) => {
        setLoadingRooms(true)
        API.post(`/test`, {
            startDate: startDate,
            endDate: endDate,
        }).then((res) => {
            const lab = Math.abs(res.data.rooms.lab - res.data.result.lab)
            const classRoom = Math.abs(res.data.rooms.classroom - res.data.result.classroom)
            const hall = Math.abs(res.data.rooms.hall - res.data.result.hall)
            console.log(lab, classRoom, hall)
            setRooms({
                lab: lab,
                classroom: classRoom,
                hall: hall,
    ...
    useEffect(() => {
        if (event.startDate && event.endDate) {
            getRoomsAvailable(event.startDate, event.endDate)
        }
    ...
    return (
        <div className="w-full text-xl">
            ...
            {
                loadingRooms && <div className="flex text-3xl font-bold items-center justify-center w-full h-full mb-3">
                    Getting the information about rooms availability...
                </div>
            }
            {
                !loading && rooms ? <div className="flex text-3xl gap-10 my-2 font-bold mb-5">
                    <div className="flex gap-2">Available Labs: {rooms.lab}</div>
                    <div className="flex gap-2">Available Classroom: {rooms.classroom}</div>
                    <div className="flex gap-2">Available Rooms: {rooms.hall}</div>
                </div> : null
            }
            <RichTextEditor
